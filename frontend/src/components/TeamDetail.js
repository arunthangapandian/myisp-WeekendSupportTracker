...
/**
 * Calculate numeric hours from a time range string.
 * @returns {number} hours, or NaN
 */
function calcHoursNum(timeStr) {
    const s = calcTotalHours(timeStr);
    if (!s) return NaN;
    return parseFloat(s);
}

/**
 * Determine support type label for UI display.
 * >= 6.5 hrs → On Call Primary Support
 * 4.5 to < 6.5 hrs → On Call Secondary Support
 */
function getSupportTypeUI(timeStr) {
    const h = calcHoursNum(timeStr);
    if (isNaN(h)) return '';
    if (h >= 6.5) return 'On Call Primary Support';
    if (h >= 4.5) return 'On Call Secondary Support';
    return '';
}
...
export default function TeamDetail({ entryId, teamId, onRefresh }) {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const saved = await api.saveAllLineItems(entryId, teamId, lineItems, empId);
    ...
    const handleExport = async () => {
        try {
            ...
            a.href = blobUrl;
            a.download = `${safeName}-${currentEntry?.date || dt}.xlsx`;
            a.click();
                            ...
                            <Table size="small" sx={{ tableLayout: 'fixed', width: '100%' }}>
                                ...
                                <TableBody>
                                    {filteredItems.map((li, i) => {
                                        const { start, end } = parseTimeParts(li.time);
                                        const hoursNum = calcHoursNum(li.time);
                                        const allowanceEnabled = !isNaN(hoursNum) && hoursNum >= 4.5;
                                        return (
