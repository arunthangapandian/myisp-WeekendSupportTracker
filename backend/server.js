...
app.put('/api/entries/:id', (req, res) => {
    ...
    if (releaseOwner !== undefined) {
        ...
        if (entry.releaseOwner !== releaseOwner.trim()) {
            changelogs[req.params.id].push({ id: uuidv4(), type: 'ownerChange', old: entry.releaseOwner, new: releaseOwner.trim(), changedBy: by, date: new Date().toISOString() });
        }
    ...
    if (date !== undefined) {
        ...
        if (entry.date !== date.trim()) {
            changelogs[req.params.id].push({ id: uuidv4(), type: 'dateChange', old: entry.date, new: date.trim(), changedBy: by, date: new Date().toISOString() });
        }
...
app.post('/api/entries/:id/sanity-upload', upload.single('sanityFile'), (req, res) => {
    ...
    if (!changelogs[req.params.id]) changelogs[req.params.id] = [];
    changelogs[req.params.id].push({ id: uuidv4(), type: 'sanityUpload', old: entry.sanityFile || '', new: req.file.filename, changedBy: by, date: new Date().toISOString() });
...
app.get('/api/entries/:id/history', (req, res) => {
    if (!entries[req.params.id]) return res.status(404).json({ error: 'Entry not found' });
    res.json(changelogs[req.params.id] || []);
});
