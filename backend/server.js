...
app.put('/api/entries/:eid/teams/:tid/line-items', (req, res) => {
    ...
    // Log team save to changelogs for Last Updated tracking
    const changedBy = (req.body.changedBy || 'UNKNOWN').trim();
...
// ── Last Updated ─────────────────────────────────────────────
// Returns recent 5 team-save events. Optional ?leadId=xxx filters to that lead's teams (for CL9).
app.get('/api/last-updated', (req, res) => {
    const leadId = (req.query.leadId || '').trim().toLowerCase();
    const all = [];
    Object.entries(changelogs).forEach(([entryId, logs]) => {
        const entry = entries[entryId];
        if (!entry) return;
        logs.forEach(h => {
            if (h.type !== 'teamSave') return;
            if (leadId && (h.leadName || '').toLowerCase() !== leadId) return;
            all.push({
                id: h.id,
                entryId,
                entryDate: entry.date,
                teamName: h.teamName || h.newValue || '',
                leadName: h.leadName || '',
 ...
 * >= 6.5 hrs → On Call Primary Support
 * 4.5 to < 6.5 hrs → On Call Secondary Support
 */
function getSupportTypeServer(hrsStr) {
    if (!hrsStr) return '';
    const num = parseFloat(hrsStr);
    if (isNaN(num)) return '';
    if (num >= 6.5) return 'On Call Primary Support';
    if (num >= 4.5) return 'On Call Secondary Support';
    return '';
}

// Generated by GitHub Copilot
/** Build a filename-safe datetime stamp: YYYY-MM-DD_HH-MM */
...
/** Capitalise every word in a string (e.g. "shift allowance" → "Shift Allowance") */
...
function styleExcelSheet(sheet, columns, rowDataList, getRowBg) {
    ...
    const nameKey = columns.find(c => c.key === 'name' || c.key === 'memberName')?.key;
    const nameCounts = {};
    if (nameKey) {
        rowDataList.forEach(r => {
            const n = (r[nameKey] || '').toLowerCase().trim();
    ...
    rowDataList.forEach((rowData, idx) => {
        ...
        const n = nameKey ? (rowData[nameKey] || '').toLowerCase().trim() : '';
        const isDuplicate = nameKey && n && nameCounts[n] > 1;
        const bgColor = isDuplicate ? DUPLICATE_BG : getRowBg(rowData, idx);
...
app.get('/api/entries/:id/export', async (req, res) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="Release-${entry.date}_${fileTimestamp()}.xlsx"`);
...
app.get('/api/entries/:eid/teams/:tid/export', async (req, res) => {
    ...
    const safeName = titleCase(team.teamName.replace(/[\\/:*?"<>|]/g, '_'));
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${safeName}-${entry.date}_${fileTimestamp()}.xlsx"`);
    await workbook.xlsx.write(res);
    res.end();
});
...
app.get('/api/entries/:id/dialog-export/:type', async (req, res) => {
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    const exportLabel = titleCase(exportType === 'allowance' ? 'Shift Allowance' : exportType === 'compoff' ? 'Compoff' : exportType === 'members' ? 'All Members' : exportType === 'teams' ? 'Teams' : exportType);
    res.setHeader('Content-Disposition', `attachment; filename="${exportLabel}-${entry.date}_${fileTimestamp()}.xlsx"`);
    await workbook.xlsx.write(res);
    res.end();
});
